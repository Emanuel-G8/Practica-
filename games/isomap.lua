...
local TREE_COLLIDER_RX = 0.32
local TREE_COLLIDER_RY = 0.20
...
function map.decodeJson(filename)
	...
	--love.filesystem.load() compiles a Lua file into a callable function,
	--without running it yet (like Lua's loadfile()).
	local chunk, err = love.filesystem.load(filename)
	if not chunk then error("Could not load map file: "..tostring(err)) end

	--Calling the chunk actually RUNS the file. Since Map.lua ends with
	--"return { ... }", calling it hands us that table back directly.
	--No parsing step needed: Lua is reading its own syntax natively.
	mapDec = chunk()
				...
				for i, objects in ipairs(mapDec.data[colunas][linhas]) do
					if objects == props.mnemonic then
						...
						table.insert(mapPropsfield,{texture=props.image, x=linhas, y=colunas, offX=props.origins[1], offY=props.origins[2],
						                           mapY = pY, mapX = pX, colX = colX, colY = colY, width = props.image:getWidth(), height = props.image:getHeight(), alpha = false})

						if props.mnemonic == "tree" then
							table.insert(treeColliders, {x = linhas, y = colunas, rx = TREE_COLLIDER_RX, ry = TREE_COLLIDER_RY})
						end		
					end				   
				end
...
function map.getTreeColliders()
	return treeColliders
end

function map.ellipseOverlap(x1, y1, rx1, ry1, x2, y2, rx2, ry2)
	local dx = x2 - x1
	local dy = y2 - y1
	local rx = rx1 + rx2
	local ry = ry1 + ry2
...
function map.collidesWithTrees(x, y, rx, ry)
	for _, t in ipairs(treeColliders) do
		if map.ellipseOverlap(x, y, rx, ry, t.x, t.y, t.rx, t.ry) then
			return true
		end
	end
	return false
